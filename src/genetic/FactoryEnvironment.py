...
def generateJob(job_id, current_time, max_time, number_abilities, max_steps, min_step_time, max_step_time, abilities,
                agent_no, statuses):
    LEEWAY = 16
    LEEWAY2 = 20
    k = random.randint(1, max_steps)
    process = random.choices(range(1, number_abilities + 1), k=k)
    times = [random.randint(min_step_time, max_step_time)*10 for _ in range(len(process))]
    job_time = sum(times)
    ...
    return Job(job_id, deadline, process, times, statuses), job


def generateJobV2(id, current_time, max_time, number_abillities, max_steps, min_step_time, max_step_time, abilities,
                  agent_no, statuses):
    LEEWAY = 16
    LEEWAY2 = 20
    k = random.randint(1, max_steps)
    ...
    for operation in process:
        ...
        for i in range(agent_no):
            if operation in abilities[i]:
                mij.append(i + 1)
                tim.append(random.randint(min_step_time, max_step_time))
            else:
                tim.append(1000)
        Mij.append(mij)
        mij = []
        max_times.append(max(i for i in tim if i < 1000))
        times.append(tim)
    ...
    return Job(id, deadline, process, times, statuses), k, Mij, Pjk
...
class Agent:
    ...
    def startJob(self, job_index):
        ...
        self.job.jobStarted()  # mark job as being processed
    ...
    def agentStep(self, current_time):
        ...
        if (self.step_time_left / self.step_total_time) < 0.1:  # less than 10% time left = finishing job
            self.agentStatus = self.STATUSES["finishing job"]
            if self.step_time_left == 0:  # step complete
                self.finishStep(current_time)
        elif (self.step_time_left / self.step_total_time) > 0.9:  # more than 90% time left = starting job
            self.agentStatus = self.STATUSES["starting job"]
        else:  # processing job
...
class Job:
    def __init__(self, id, deadline, process, times, statuses):
        ...
        self.current_step = self.PROCESS[0]  # current step requried to be executed
        self.current_step_time = self.TIMES[0]  # time for current step
    ...
    def stepComplete(self, current_time):
        ...
        self.current_step = self.process_left[0]  # update step required to be worked on
        self.current_step_time = self.times_left[0]  # update time required for step
...
class Factory(gym.Env):
    def __init__(self, no_agents, abilities, conveyor_size, conveyor_fill, conveyor_pos, episode_steps, no_jobs,
                 action_space, job_statuses, agent_statuses, operation_size, window_size, job_sequence, op_sequence):
        ...
        self.last_job_id = 0  # keeps track of job id
    ...
    def getProcessObservation(self, job):
        ...
        if len(process_observation) >= n:
            ...
            process_observation = process_observation + [process_observation[-1]] * (n - len(process_observation))
        ...
        for agent in self.agents:
            ...
            if agent.getStatus() is self.AGENT_STATUSES["idle"]:  # observe conveyor belt if agent idle
                ...
                for i in range(self.WINDOW_SIZE):
                    ...
                    if job is None:  # if no job, observe no job
                        ...
                        agent_observation = agent_observation + [-1 for _ in range(self.OPERATION_SIZE)]
                    else:  # observe job process (n steps, if job seen)
                        agent.look_job.append(job)
                        process_observation = self.getProcessObservation(job)
            ...
            #  else agent is busy working or deciding whether to unload so cannot make observation of jobs on conveyor
                ...
                if agent.getStatus() is self.AGENT_STATUSES["ready for unload"]:
                    process_observation = self.getProcessObservation(agent.job)
                else:
                    process_observation = [-1 for _ in range(self.OPERATION_SIZE)]
                agent_observation = agent_observation + process_observation
    ...
    def step(self, actions, current_time, show, jobs):

        for agent in self.agents:
            agent.doAction(actions[agent.ID], current_time)  # each agent to do action
        # job addition
        if (self.last_job_id < self.NUMBER_JOBS):
            self.conveyor_buffer.append(jobs[self.last_job_id])
            self.last_job_id += 1
        ...
        observations, masks = self.getObservations()
        return observations, masks, rewards, dones

    def _getReward(self, current_time):
        # define reward values

        COMPLETE_ON_TIME_REWARD = 50
        WORKING_REWARD = 10
        ...
        for agent in self.agents:
            if len(self.completed) >= self.NUMBER_JOBS:
                dones[agent.ID] = True
            if agent.job and (agent.job.getJobStatus() == self.JOB_STATUSES["finished"]):
                reward = 0  # reward for completion by deadline
                share_reward = True
                agent.agentReset()
            elif agent.getStatus() != 0:
                reward = WORKING_REWARD  # reward for working
            else:
                reward = 0
        ...
        if share_reward:
            for i in range(len(rewards)):
                if share_reward:
                    rewards[i] = rewards[i] + COMPLETE_ON_TIME_REWARD - 0.1*current_time
                    # shared on-time completion reward
    ...
    def display(self, step):
        ...
        for pos in self.conveyor_pos[1:]:
            agent_places[pos] = f'{self.agents[i].ID}A' if self.agents[i].getStatus() == 0 else str(i) + str(self.agents[i].job.ID)
            i += 1
